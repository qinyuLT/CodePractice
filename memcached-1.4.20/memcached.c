...
static void settings_init(void) {
    ...
    settings.factor = 1.25;  //chunk ´óÐ¡µÄÔö³¤Òò×Ó
...
//½«fd×¢²áµ½libevent:×îºó¿ÉÒÔ¿´µ½£¬µ÷ÓÃÒÔÏÂÈý¸öº¯Êý
                ...
                struct event_base *base) 
{
    ...
    c->noreply = false;
//event_set µ÷ÓÃ»Øµ÷º¯Êýevent_handler
...
static void out_string(conn *c, const char *str) {
    ...
    conn_set_state(c, conn_write);//½øÈë×´Ì¬»ú
...
static void complete_nread_ascii(conn *c) {
    ...
    if (strncmp(ITEM_data(it) + it->nbytes - 2, "\r\n", 2) != 0) {
      ...
      switch (ret) {
      case STORED:
          out_string(c, "STORED");//Ïò¿Í»§¶ËÊä³öÐÅÏ¢
          break;
      case EXISTS:
          out_string(c, "EXISTS");
          break;
...
static void reset_cmd_handler(conn *c) {
    ...
    if(c->item != NULL) {//item ´æÔÚ£¬¼´ÊÇÖØÖÃÃüÁî£¬É¾³ýÖ®Ç°µÄitem
        item_remove(c->item);
        c->item = NULL;
    }
    conn_shrink(c);//å???
    if (c->rbytes > 0) {
        ...
        conn_set_state(c, conn_waiting);//µÚÒ»´ÎÇëÇó»á½øÈë
    }
}
...
static void process_update_command(conn *c, token_t *tokens, const size_t ntokens, int comm, bool handle_cas) {
    ...
    key = tokens[KEY_TOKEN].value;//¼üÃû
    nkey = tokens[KEY_TOKEN].length;//¼ü³¤¶È
...
//ÄÚ´æ¹ÜÀí·ÖÅä¹¤×÷
    ...
    c->rlbytes = it->nbytes;
    c->cmd = comm;
    conn_set_state(c, conn_nread);//¼ÌÐøÖ´ÐÐ×´Ì¬»ú£¬case conn_nread,´¦ÀíÃüÁîÐÐÊý¾Ý²¿·Ö
}
...
//ÏÂÃæÕâ¸ötokenize_commandÊÇÒ»¸ö´Ê·¨·ÖÎö£¬°Ñcommand·Ö½â³ÉÒ»¸ö¸ötoken
    ntokens = tokenize_command(command, tokens, MAX_TOKENS);
//ÏÂÃæÊÇ¶ÔÉÏÃæ·Ö½â³öÀ´µÄtokenÔÙ½øÐÐÓï·¨·ÖÎö£¬½âÎöÃüÁî£¬ÏÂÃæµÄcomm±äÁ¿Îª×îÖÕ½âÎö³öÀ´ÃüÁîÀàÐÍ
    if (ntokens >= 3 &&
   ...
   {
//add/set/replace/prepend/appendÎª¡°¸üÐÂ¡±ÃüÁî£¬µ÷ÓÃÍ¬Ò»¸öº¯ÊýÖ´ÐÐÃüÁî¡£Ïê¼ûprocess_update_command¶¨Òå´¦
        process_update_command(c, tokens, ntokens, comm, true);//Ö»ÊÇ´¦ÀíÃüÁîµÄÒ»²¿·Ö
...
static int try_read_command(conn *c) {
    ...
    if (c->protocol == binary_prot) {
        ...
        if (c->rbytes == 0)//¶ÁbufferÃ»ÓÐ´ý½âÎöµÄÊý¾Ý
            return 0;

        el = memchr(c->rcurr, '\n', c->rbytes);//ÕÒµÚÒ»¸öÃüÁîµÄÄ©Î²£¬¼´»»ÐÐ·û
        if (!el) {
            if (c->rbytes > 1024) {
                /*
...
/*  Èç¹ûÃ»ÓÐÕÒµ½»»ÐÐ·û£¬ÔòËµÃ÷¶Áµ½µÄÊý¾Ý»¹²»×ãÒÔ³ÉÎªÒ»¸öÍêÕûµÄÃüÁî£¬·µ»Ø0 */
            return 0;
        }
        cont = el + 1;//ÏÂÒ»¸öÃüÁîµÄ¿ªÍ· 
        ...
        *el = '\0';
        ...
        process_command(c, c->rcurr);//Ö´ÐÐÃüÁî¡£
 //µ±Ç°ÃüÁîÖ´ÐÐÍêÖ®ºó£¬°Ñµ±Ç°Ö¸ÕërcurrÖ¸Ïò ÏÂÒ»¸öÃüÁîµÄ¿ªÍ·£¬²¢µ÷ÓÃrbytes£¨Ê£ÓàÎ´´¦Àí×Ö½ÚÊý´óÐ¡£©
 //Âß¼­ÉÏÏàµ±ÓÚ°ÑÒÑ´¦ÀíµÄÃüÁîÈ¥µô¡£
 ...
 */
 /*
 try_read_networkº¯Êý¾ÍÊÇ´ÓsocketÖÐ°ÑÊý¾Ý¶Áµ½c->rbufÖÐÈ¥¶øÒÑ£¬
 Í¬Ê±³õÊ¼»¯Ò»Ð©±äÁ¿ÀýÈçrbytesµÈ£¬¶ÁÈ¡Êý¾Ý³É¹¦Ôò·µ»ØREAD_DATA_RECEIVED£¬
 ×´Ì¬»ú conn_set_state(c, conn_parse_cmd);½øÈëconn_parse_cmd×´Ì¬£º
*/
static enum try_read_result try_read_network(conn *c) {
...
static enum transmit_result transmit(conn *c) {
    ...
    if (c->msgcurr < c->msgused) {
        ...
        res = sendmsg(c->sfd, m, 0);//×îÖÕµ÷ÓÃsendmsgº¯Êý£¬°Ñ"STORED"×Ö·û´®ÏìÓ¦¸ø¿Í»§¶Ë
        if (res > 0) {
            pthread_mutex_lock(&c->thread->stats.mutex);
...
/*drive_machineº¯Êý:×´Ì¬»úË¼Ïë
Ò»¸öwhileÑ­»·ÀïÃæÓÐÒ»¸ö¾Þ´óµÄswitch case£¬¸ù¾ÝÁ¬½Ó¶ÔÏó connµ±Ç°µÄÁ¬½Ó×´Ì¬conn_state£¬
½øÈë²»Í¬µÄcase£¬¶øÃ¿¸öcase¿ÉÄÜ»á¸Ä±äconnµÄÁ¬½Ó×´Ì¬£¬Ò²¾ÍÊÇËµÔÚÕâ¸öwhile+switchÖÐ£¬
conn»á²»¶ÏµÄ·¢Éú×´Ì¬×ªÒÆ£¬×îºó±»·Ö·¢µ½ºÏÊÊµÄcaseÉÏ×÷´¦Àí¡£
¿ÉÒÔÀí½âÎª£¬ÕâÀïÊÇÒ»¸öÓÐÏòÍ¼£¬Ã¿¸öcaseÊÇÒ»¸ö¶¥µã£¬
ÓÐÐ©caseÍ¨¹ý¸Ä±äconn¶ÔÏóµÄÁ¬½Ó×´Ì¬ÈÃ³ÌÐòÔÚÏÂÒ»´ÎÑ­»·ÖÐ½øÈëÁíÒ»¸öcase£¬
¼¸´ÎÑ­»·ºó³ÌÐò×îÖÕ½øÈëµ½¡°ÎÞ³ö¶ÈµÄ¶¥µã¡±È»ºó½áÊø×´Ì¬»ú£¬
ÕâÀïµÄÎÞ³ö¶ÈµÄ¶¥µã¾ÍÊÇ´øÉèÖÃstop=trueµÄcase·ÖÖ§¡£
*/
static void drive_machine(conn *c) {
    bool stop = false;
    int sfd;
    ...
    while (!stop) {
//Í¨¹ýµ±Ç°Á´½ÓµÄ×´Ì¬state½øÐÐÏàÓ¦´¦Àí
//if( conn_listening ) µ÷ÓÃ¸Ãº¯Êý dispatch_conn_new()
/*Ö÷Ïß³Ì×´Ì¬»úµÄÐÐÎªÎÒÃÇÒÑ¾­ÖªµÀÁË£¬ÓÀÔ¶Ö»»áÊÇconn_listening×´Ì¬£¬
ÓÀÔ¶Ö»»á½øÈëdrive_machineµÄconn_listening·ÖÖ§£¬
acceptÁ¬½Ó°Ñclient fd Í¨¹ýdispatch_conn_newº¯Êý·Ö·¢¸øworkerÏß³Ì¡£
*/
        switch(c->state) {
        case conn_listening:      //conn_listening
            addrlen = sizeof(addr);
#ifdef HAVE_ACCEPT4
            if (use_accept4) {
            ...
            if (settings.maxconns_fast &&
                ...
                STATS_UNLOCK();
            } else {//dispatch_conn_new º¯Êýµ÷ÓÃ,·Ö·¢client fd¸øworkerÏß³Ì
            /*
            µ±Ö÷Ïß³Ìµ÷ÓÃdispatch_conn_newµÄÊ±ºò£¬workerÏß³Ì´´½¨conn¶ÔÏó£¬
            ³õÊ¼×´Ì¬Îªconn_new_cmd¡£ËùÒÔµ±ÓÐworkerÏß³Ì¼àÌýµÄclient fdÓÐÇëÇó¹ýÀ´Ê±£¬
            ÀýÈç¿Í»§¶Ë·¢ÁËÒ»ÐÐÃüÁî£¨set xxx\r\n£©»á½øÈëconn_new_cmd·ÖÖ§
			*/
...
//case conn_waiting
        case conn_waiting:
            ...
            stop = true;
            break;
//case conn_read:½øÈëtry_read_networkº¯Êý£¬¶Á³öÇëÇó
        case conn_read:
            res = IS_UDP(c->transport) ? try_read_udp(c) : try_read_network(c);
	//try_read_networkº¯ÊýµÄ·µ»ØÖµ£¬½øÈëswitch
            switch (res) {
            case READ_NO_DATA_RECEIVED:
                conn_set_state(c, conn_waiting);
                break;
            case READ_DATA_RECEIVED://ÊÇÃüÁî
                conn_set_state(c, conn_parse_cmd);
                break;
            case READ_ERROR:
                conn_set_state(c, conn_closing);
        ...
        /**
            try_read_networkºó£¬µ½´ïconn_parse_cmd×´Ì¬£¬µ«try_read_network²¢²»È·±£Ã¿´Îµ½´ï
            µÄÊý¾Ý¶¼×ã¹»Ò»¸öÍêÕûµÄcmd£¨asciiÐ­ÒéÇé¿öÏÂÍùÍùÊÇÃ»ÓÐ"\r\n"£¬¼´»Ø³µ»»ÐÐ£©£¬
            ËùÒÔÏÂÃæµÄtry_read_commandÖ®ËùÒÔ½Ðtry¾ÍÊÇÕâ¸öÔ­Òò£¬
            µ±¶Áµ½µÄÊý¾Ý»¹²»¹»³ÉÎªÒ»¸öcmdµÄÊ±ºò£¬·µ»Ø0£¬conn¼ÌÐø½øÈëconn_waiting×´Ì¬µÈ´ý¸ü¶àµÄÊý¾Ýµ½´ï¡£
 *///´¦ÀíÃüÁî
 //case conn_parse_cmd
        case conn_parse_cmd :
            if (try_read_command(c) == 0) {//¸Ãº¯Êýµ÷ÓÃprocess_command£¬½âÎöÃüÁî²¢Ö´ÐÐ
                /* wee need more data! */
                conn_set_state(c, conn_waiting);
            }
...
//case conn_new_cmd
        case conn_new_cmd:
            /* Only process nreqs at a time to avoid starving other
               connections */
			/*
             ÕâÀïµÄreqsÊÇÇëÇóµÄÒâË¼£¬ÆäÊµ½Ð¡°ÃüÁî¡±¸ü×¼È·¡£Ò»´Îevent·¢Éú£¬ÓÐ¿ÉÄÜ°üº¬¶à¸öÃüÁî£¬
             ´Óclient fdÀïÃæreadµ½µÄÒ»´ÎÊý¾Ý£¬²»ÄÜ±£Ö¤Õâ¸öÊý¾ÝÖ»ÊÇ°üº¬Ò»¸öÃüÁî£¬ÓÐ¿ÉÄÜÊÇ¶à¸ö
             ÃüÁîÊý¾Ý¶ÑÔÚÒ»ÆðµÄÒ»´ÎÊÂ¼þÍ¨Öª¡£Õâ¸önreqsÊÇÓÃÀ´¿ØÖÆÒ»´Îevent×î¶àÄÜ´¦Àí¶àÉÙ¸öÃüÁî¡£
            */
            --nreqs;
            if (nreqs >= 0) {
			/**
                ×¼±¸Ö´ÐÐÃüÁî¡£ÎªÊ²Ã´½Ðreset cmd£¬reset_cmd_handlerÆäÊµ×öÁËÒ»Ð©½âÎöÖ´ÐÐÃüÁîÖ®Ç°
                µÄ³õÊ¼»¯¶¯ÏÂÒ»¸ö£¬¶¼»áÖØÐÂ½øÈëÕâ¸öcase×÷¡£¶øÏñÉÏÃæËµµÄ£¬Ò»´ÎeventÓÐ¿ÉÄÜÓÐ¶à¸öÃüÁî£¬Ã¿Ö´ÐÐÒ»¸öÃüÁî£¬Èç¹û»¹ÓÐ
                 conn_new_cmd£¬resetÒ»ÏÂÔÙÖ´ÐÐÏÂÒ»¸öÃüÁî¡£
                */
                reset_cmd_handler(c);//µ±client fd µÚÒ»´ÎÇëÇóÊ±£¬»á½øÈë
            } else {
                pthread_mutex_lock(&c->thread->stats.mutex);
            ...
            break;
//case conn_nread:
        case conn_nread://´¦ÀíÃüÃûµÄ"Êý¾ÝÐÐ" ²¿·Ö
            if (c->rlbytes == 0) {
			/**
                ×¢ÒârlbytesÎª0²Å¶ÁÍê£¬·ñÔò×´Ì¬»úÒ»Ö±»á½øÀ´Õâ¸öconn_nread·ÖÖ§¼ÌÐø¶ÁvalueÊý¾Ý£¬
                ¶ÁÍê¾Íµ÷ÓÃcomplete_nreadÍê³ÉÊÕÎ²¹¤×÷£¬³ÌÐò»á¸ú×Åcomplete_nread½øÈëÏÂÒ»¸ö
                ×´Ì¬¡£ËùÒÔÖ´ÐÐÍêcomplete_nread»ábreak; 
                */
                complete_nread(c);//½øÈë
                break;
            }
 //Èç¹û»¹ÓÐÊý¾ÝÃ»¶ÁÍê£¬¼ÌÐøÍùÏÂÖ´ÐÐ¡£¿ÉÖª£¬ÏÂÃæµÄ¶¯×÷¾ÍÊÇ¼ÌÐø´ÓbufferÖÐ¶ÁvalueÊý¾ÝÍùitemÖÐµÄdataµÄvalueÎ»ÖÃÈû¡£
...
//case conn_mwrite
        case conn_mwrite:
          if (IS_UDP(c->transport) && c->msgcurr == 0 && build_udp_headers(c) != 0) {
            ...
            switch (transmit(c)) {//½øÈëtransmit()º¯Êý
...
//memecachedÆô¶¯Èë¿Úmainº¯Êý
int main (int argc, char **argv) {
    int c;
    bool lock_memory = false;
    bool do_daemonize = false;//ÊØ»¤½ø³Ì -d
    bool preallocate = false;//Ô¤·ÖÅä±ê¼Ç -L
    int maxcore = 0;
    char *username = NULL;
    char *pid_file = NULL;
    struct passwd *pw;
    ...
    while (-1 != (c = getopt(argc, argv,
        ...
        case 'L' :
            if (enable_large_pages() == 0) {
                preallocate = true;//Ô¤·ÖÅä±ê¼Ç ¸Ä±ä
            } else {
...
//³õÊ¼»¯Ö÷Ïß³ÌµÄlibevent
    ...
    conn_init();
//ÄÚ´æ¹ÜÀí³õÊ¼»¯º¯Êý (×î´óÄÚ´æÄ¬ÈÏ64M£¬Ôö³¤Òò×Ó£¬Ô¤·ÖÅä±ê¼Ç)
...
//server_sockets ´´½¨¼àÌýsocket
        errno = 0;
        if (settings.port &&//Æô¶¯Ö÷Ïß³Ì  TCP·½Ê½
        ...
        errno = 0;
        if (settings.udpport && //Æô¶¯Ö÷Ïß³Ì UDP
...
//ÕâÊ±Ö÷Ïß³ÌÆô¶¯£¬¿ªÊ¼Í¨¹ýlibevent½ÓÊÜÍâ²¿ÇëÇó
