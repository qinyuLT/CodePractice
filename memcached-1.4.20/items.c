...
//根据item大小，寻找合适的页号(slabclass数组的下标)
    ...
    search = tails[id];//LRU队列的尾部
 //首先从LRU队列找过期的item，试探5次
 //否则重新分配空间 slabs_alloc
 //否则替代LRU队列最后一次item的空间
...
//delete<item>，get<item>时检测到过期了，释放空间
void item_free(item *it) {
    ...
    it->slabs_clsid = 0;
    DEBUG_REFCNT(it, 'F');
    slabs_free(it, ntotal, clsid);//调用该函数
}
