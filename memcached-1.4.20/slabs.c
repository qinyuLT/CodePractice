...
void slabs_init(const size_t limit, const double factor, const bool prealloc) {
    ...
    if (prealloc) {
        ...
        mem_base = malloc(mem_limit);//一次性分配64M空间
        if (mem_base != NULL) {
...
//内存分配的规则
    while (++i < POWER_LARGEST && size <= settings.item_size_max / factor) {
        ...
        if (size % CHUNK_ALIGN_BYTES)//块的大小按照8字节对齐
...
static void slabs_preallocate (const unsigned int maxslabs) {
	...
	{
        if (++prealloc > maxslabs)//从1到最大块的序号 依次划分空间
            return;
		//给第i块分配空间
...
//页数组的初始化为16，以后则2倍增长
//虽然初始化为16页，但真正得到内存的只有第1页，需要时才会将真的内存分配给接下来的页
...
static int do_slabs_newslab(const unsigned int id) {
    slabclass_t *p = &slabclass[id];
//如果settings.slab_reassign标记为真，才一次性增长1M空间
//否则 size*perslab <= 1M
...
//在这里可以看到，页的有效大小slabs只++一次，并不是+= list_size(16)次
//即虽然初始化为16页，但真正得到内存的只有1页，需要时才会将真的内存分配给接下来的页
...
//如果当前没有空闲空间，则do_slabs_newslab分配
