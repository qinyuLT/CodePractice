...
struct conn {
    ...
    short  which;   /** which events were just triggered *///刚触法的事件
 
 /**
 	读buffer会涉及两个方向上的“读”：
 	一个是从socket读进来到rbuf里面
 	一个是从rbuf里面把数据读出去解析，读buffer相当于一个中介，
 	从socket读进来最终还是得被别人读出去解析，而
 	rcurr工作指针与rbytes就是在rbuf数据被读出去解析的时候用到，
 	用来控制可以读出去被解析的数据还剩余多少。
 */
    char   *rbuf;   /** 读buffer */
    char   *rcurr;  /** 读buffer的当前指针 */
    int    rsize;   /** 读buffer的大小 */
    int    rbytes;  /** 剩余buffer的字节数*/
